...
/* var_dump($photo);die;*/
      if(empty($data['username'])){
            echo '<script>alert("请输入充值mikey账号"); window.history.back(-1); </script>';
...
public function cation()
	...
	public function index()
	{
        ...
        //总后台商品还是个人店铺商品，0为总后台
        $shang['shangjia']=["neq",0];
	    //热门商品
	    //根据添加时间排序 降序 由最新添加的  返回6条数据
	 
	    $hot_product_list = M("product_detail")->where(array("is_hot"=>1))->where($shang)->limit(6)->order("is_sort desc")->select();
	    $this->assign("hot_product_list",$hot_product_list);

        //最新产品
        $new_product_list = M("product_detail")->where(array("is_new"=>1))->where($shang)->order("is_sort desc")->limit(6)->select();
        $this->assign("new_product_list",$new_product_list);


        //母婴用品
        $mu_product_list = M("product_detail")->where(array("status"=>1))->where($shang)->order("is_sort asc")->limit(8)->select();
        $this->assign("mu_product_list",$mu_product_list);

        //为您推荐
        $like_product_list = M("product_detail")->where(array("is_tuijian"=>1))->where($shang)->order("is_sort desc")->limit(8)->select();
...
// }
        ...
        // $data['jifen_nums'] = I('jifen_nums');//赠送数量
