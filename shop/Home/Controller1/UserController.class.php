...
class UserController extends CommonController
{
   public function Personal()
    {
        $uid = session('userid');
        $uinfo = M('user')->where(array('userid' => $uid))->field('username,is_dailishang,userid,img_head,use_grade,user_credit,use_grade')->find();
   ...
   public function Mobile()
    ...
    public function News()
    {
        ...
        foreach ($newinfo as $key =>$val){
            $newinfo[$key]['content'] = html_entity_decode($val['content'], ENT_QUOTES, 'UTF-8');
        }
    ...
    public function Sharecode()
    {
        ...
        $uinfo = M('user')->where(array('userid' => $userid))->field('username,mobile,is_dailishang,userid,img_head,use_grade,user_credit,vip_grade')->find();
...
//       if (!file_exists($drpath . '/' . $imgma)) {
        ...
        $aurl = "https://".$_SERVER['SERVER_NAME']. U('Login/register/mobile/' . $uinfo['mobile']);
