...
//卖入中心
    public function Buycenter(){
        if(IS_AJAX){
            ...
            $order_info = M('trans as tr')->join('LEFT JOIN  ysk_user as us on tr.payout_id = us.userid')->where(array('tr.pay_state'=>0,'tr.trans_type'=>1,'tr.pay_nums'=>$pricenum,'tr.payout_id'=>array('gt',0)))->order('id desc')->select();
            foreach($order_info as $k => $v){
                $order_info[$k]['cardinfo'] = M('bank_name bn')->join('LEFT join ysk_ubanks as ub on bn.q_id = ub.card_id')->where(array('ub.id'=>$v['card_id']))->getfield('banq_genre');
