    ...
    $type=$mydeal["trans_type"];
     ...
     try{
         ...
         if($type==1){//卖出单，删除订单
                ...
                if($res1){
                    $doDecOne = M('store')->where(array('uid'=>$uid))->setInc('cangku_num',$sellnums);
                    //增加自己的余额记录
                    $pay_n = M('store')->where(array('uid' => $uid))->getfield('cangku_num');
                    //添加余额记录
                   $One = addAccountRecords($uid,0,$sellnums,12,$pay_n,$type = 'money');
                    $doDecTwo = M('store')->where(array('uid'=>$uid))->setInc('cangku_num',$shouxufei);
                    //增加自己的余额记录
                    $pay_n = M('store')->where(array('uid' => $uid))->getfield('cangku_num');
                    //添加余额记录
                    $Two = addAccountRecords($uid,0,$shouxufei,24,$pay_n,$type = 'money');
                    if( $doDecOne && $doDecTwo && $One && $Two){
                        M()->commit();
                        ajaxReturn(L('qxcg'),1);
                        return;
                    }else{
             ...
             //增加自己的余额记录
               ...
               //添加余额记录
                 $One =  addAccountRecords($uid,0,$sellnums,12,$pay_nOne,$type = 'money');
                 $pay_nTwo = M('store')->where(array('uid' => $uid))->getfield('cangku_num');
                 //添加余额记录
                 $Two = addAccountRecords($uid,0,$shouxufei,24,$pay_nTwo,$type = 'money');
                  $payout['payout_id'] =0;
                  $payout['pay_state'] =0;
                  $res1 = M('trans')->where(array('id'=>$id))->save($payout);
                 if($doDec  && $One && $Two && $res1){
                     M()->commit();
                     ajaxReturn(L('qxcg'),1);
                     return;
                 }else{
...
//卖出记录
    ...
    public function Con_Getmoney(){
        ...
        $payin_id = D('user')->where('userid='.$order_info["payin_id"])->field('is_center')->find()['is_center'];
        $payout_id = D('user')->where('userid='.$order_info["payout_id"])->field('is_center')->find()['is_center'];
        //添加买入余额记录
            $One =  addAccountRecords($order_info['payin_id'],$order_info['payout_id'],$paynums,11,$pay_n,$type = 'money');
            if(!$res_pay || !$One ){
                // 回滚事务
                M()->rollback();
                $arr =['res_pay'=>$res_pay,'One'=>$One];
                ajaxReturn($arr,0);
                return;
            }
        //判断是买入还是卖出单
     /*   if($order_info['trans_type'] == 0){
            ...
            $pay_n = M('store')->where(array('uid' => $order_info["payin_id"]))->getfield('cangku_num');
            //添加买入余额记录
            $arr =  addAccountRecords($order_info['payin_id'],$order_info['payout_id'],$backnums,22,$pay_n,$type = 'money');
            if(!$arr){
                // 回滚事务
                M()->rollback();
                $arr = ['arr'=>$arr];
        ...
        //直推释放（一级），加一条余额记录
        if($payin_id!=3 && $payin_id!=1 && $payout_id!=3  && $payout_id!=1){
            $status=  userAward($order_info['payin_id'],'zhitui',1,$paynums);
            if(!$status){
                M()->rollback();
                $arr =['status'=>$status];
                ajaxReturn($arr,0);
        ...
        //修改交易时间 一天只能卖出一次
        $thisTime['selltime'] = strtotime(date('Y-m-d 00:00:00',time()));
        $updataTime =  D('user')->where("userid=".$order_info['payout_id'])->save($thisTime);

            if($res_suc  && $updataTime){
                  //提交事务
                 M()->commit();
                ...
                // 回滚事务
                M()->rollback();
                $arr=['remit'=>$res_suc,'status'=>$status,'updataTime'=>$updataTime];
                ajaxReturn($arr,0);
            }
        }catch (\Exception $e) {
            // 回滚事务
            M()->rollback();
            ajaxReturn(json_decode($e),0);
            return;
        }
    }
...
//                $order_info[$k]['spay'] = $v['pay_nums'] * 0.85;
    ...
    public function Dosells(){
        if(IS_AJAX){
            ...
            $pwd = trim(I('pwd'));
            $sellnums = M('trans')->where(array('id'=>$trid))->field('pay_nums,payin_id,pay_state,pay_type')->find();

            //1银行卡2,支付宝3微信4imtoken
           if($sellnums['pay_type'] == 1){
               $where['user_id'] = $uid;
               $where['is_default'] = 1;
               $id_setcards = D('ubanks')->where($where)->field("id")->find();
           }else if($sellnums['pay_type'] == 2){
               $where['userid'] = $uid;
               $where['alipay_default'] ="1";
               $id_setcards = D('alipay')->where($where)->field("id")->find();
           }else if($sellnums['pay_type'] == 3){
               $where['userid'] = $uid;
               $where['weichar_default'] ="1";
               $id_setcards = D('weichar')->where($where)->field("id")->find();
           }else if($sellnums['pay_type'] == 4){
               $where['userid'] = $uid;
               $where['imtoken_default'] ="1";
               $id_setcards = D('imtoken')->where($where)->field("id")->find();
           }
            $sellAll = array(500,1000,3000,5000,10000,30000);
            ...
            $allNum = bcadd($shouxufei,$shouldpay,2);
               ...
               /* $id_setcards = M('ubanks')->where(array('user_id'=>$uid,'is_default'=>1))->count('1');
                ...
                $doDecOne = M('store')->where(array('uid'=>$uid))->setDec('cangku_num',$shouldpay);
      
                $pay_n = M('store')->where(array('uid' => $uid))->getfield('cangku_num');
                  //添加余额记录
                 $One = addAccountRecords($uid,0,-$shouldpay,12,$pay_n,$type = 'money');
                 $doDecTwo = M('store')->where(array('uid'=>$uid))->setDec('cangku_num',$shouxufei);
                 $pay_n = M('store')->where(array('uid' => $uid))->getfield('cangku_num');
                 //添加余额记录
                 $Two = addAccountRecords($uid,0,-$shouxufei,23,$pay_n,$type = 'money');
                //记录买入会员且修改订单状态
                $res_Buy = M('trans')->where(array('id'=>$trid))->setField(array('payout_id'=>$uid,'pay_state'=>1,'card_id'=>$id_setcards['id'],'fee_nums'=>$shouxufei));
                /*//修改交易卖出时间一天只能卖一次
                $thisTime['selltime'] = strtotime(date('Y-m-d 00:00:00',time()));
                $updataTime =  D('user')->where("userid=$uid")->save($thisTime);*/
                if($doDecOne && $One && $doDecTwo && $Two &&$res_Buy){
                    //提交事务
                    M()->commit();
                    ajaxReturn(L('mccg'),1);
                    return;
