...
class SettingsController extends CommonController
{
	...
	public function dltwen(){
		...
		if($id=4 || $id=5 || $id=6 || $id=13){
           echo '<script>alert("帮助中心、mikey介绍、商城介绍、联系客服 根据需求删除!"); window.history.back(-1); </script>';
		}else{
	...
	public function Savewentype()
	{
		...
		if ($id != '') {
			if(3<=$id && $id<=6){
				echo '<script>alert("帮助中心、mikey介绍、商城介绍、联系客服 根据需求修改!"); window.history.back(-1); </script>';
			}else{
				$res = $configsAll->where(array('id' => $id))->save($data);
